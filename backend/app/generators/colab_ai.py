...
class ColabAiGenerator(VideoGenerator):
    ...
    Interface-only for now (see project README): fully wired to
    google-colab-cli's documented commands (`colab new/exec/download/stop`),
    but google-colab-cli only runs on Linux/macOS with an authenticated
    Google/Colab account, neither of which this dev environment has. On any
    other platform this raises colab_client.ColabUnavailableError, which the
    job runner surfaces as a normal failed-job message.
    """
